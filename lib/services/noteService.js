...
const {insertNote, deleteNote, findNoteById, getNotesForUser} = require("../repositories/noteRepository");
...
async function getUsersNotesUrl(username) {
    const notes = await getNotesForUser(username);
    return notes.map((note) => note.pageUrl);
}
